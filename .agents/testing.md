...
It needs a built `dist` and a scratch database (it drops the target). On **MariaDB** two of its eleven checks fail on drift and that is expected: typeorm's mysql driver models MySQL, so a MariaDB schema diverges from the entity metadata by construction (a native `uuid` type from 10.7 onwards, among others). Measured against the current chain it is 292 statements before the alignment migration and 213 after, so the migration reduces the divergence rather than causing it. The data-integrity checks — row counts, column values, foreign keys rejecting orphans and cascading — all pass on MariaDB, which is what the gate is there to certify. It runs in the `tests-migrations` job as the final step, for both dialects, because it is the only gate that can certify a migration touching columns, constraints or rows — the empty round-trip above cannot. Run it locally too when authoring such a migration, rather than waiting for CI.
