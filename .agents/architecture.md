...
- **No `identity`: the caller.** `POST /permissions/:id/check` evaluates on `actor.permissionEvaluator`, which on a request IS the wrapper above and attaches the caller's identity under `global`. `POST /policies/:id/check` runs the policy engine without a permission evaluator, so its controller hands `useRequestPolicyIdentity(event)` to `PolicyCheckerService` as the `caller`. A bearer without `global` is therefore the only way a check runs anonymously.
