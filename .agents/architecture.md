...
**The two single-subject check routes reach it without a rule of their own (#3604).** `POST /permissions/:id/check` evaluates on `actor.permissionEvaluator`, which on a request IS this wrapper, so `PermissionCheckerService` builds no evaluator and places no identity. `POST /policies/:id/check` has no permission evaluator to route through (it runs the policy engine on one policy), so its controller applies the function to the bag and `PolicyCheckerService` evaluates the bag exactly as given. On both routes a body `identity` is therefore overwritten or removed, an explicit `null` included, and a bearer without `global` is how a check runs anonymously. That matters beyond scopes: the permission-binding evaluator loads the grants of whatever identity the bag names, and on the policy route the caller also supplies `permissionBinding`, so an honoured body identity answered for another subject, cross-realm, to any authenticated caller. Wrapping the policy in a memory-provider `PermissionEvaluator` instead was rejected: it overwrites a caller's `permissionBinding` and turns `PolicyError` into `PermissionError`. Pinned by the *#3604* cases in `test/unit/http/controllers/entities/{permission,policy}/checker.spec.ts`.
