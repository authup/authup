...
**`applyRequestIdentity(event, data)` is the one place the `global`-scope condition is spelled, and it is SYMMETRICAL.** It sets `IDENTITY` to the request's own identity when the scope permits (overwriting whatever a caller put there, so an identity can never be injected past the resolution), and DELETES the key when it does not. `extendContext` passes every evaluation's bag through it, so a caller may fill the bag itself (the batch authorization check does, so every identity-reading policy in a tree gets the data) without restating the scope rule. An attach-only version silently held the gate for callers that left the key empty and lost it for any that did not, which is one edit away at every new call site. Only the identity key is governed; the rest of the caller's bag rides through untouched. Pinned by *should remove a pre-placed identity without global scope* and *should overwrite a pre-placed identity with the request's own* (`test/unit/adapters/http/request/permission.spec.ts`).

**The two single-subject check routes ask about the caller through it, and about anyone else only under `permission_check` (#3604).** Both controllers take the body's `identity` out of the data and hand it to the checker as the `subject`.

- **No subject (absent or `null`): the caller.** `POST /permissions/:id/check` evaluates on `actor.permissionEvaluator`, which on a request IS this wrapper. `POST /policies/:id/check` has no permission evaluator to route through (it runs the policy engine on one policy), so its controller applies the function to the data and `PolicyCheckerService` evaluates it as given. A bearer without `global` is therefore the only way a check runs anonymously.
- **A subject `{ type, id }`: that user or client, by UUID only**, because names repeat across realms (every realm has an `admin-console` client). `resolveCheckSubject` (`core/identity/permission/checker/subject.ts`) pre-evaluates `PERMISSION_CHECK`, loads the subject through the identity resolver, then evaluates the permission with `realmMatch` set to the subject's STORED realm, so a `realm_admin` checks users of its own realm only and a realm the caller writes is never consulted. The subject's identity is then evaluated as stored: on a bare `PermissionEvaluator` for the permission route, since the wrapper would swap the caller back in, and set into the data for the policy route. Being refused the subject throws (400 malformed, 403 gate, 404 unknown) rather than answering `status: "error"`, since it answers the request, not the check. The gate is pre-evaluated before the lookup, so a caller holding no grant learns nothing about which subjects exist.

The gate exists because the permission-binding evaluator loads the grants of whatever identity it is handed, and on the policy route the caller also supplies `permissionBinding`, so an ungated subject would let any authenticated caller read anyone's authorization. It is a dedicated permission rather than the grant-read permissions (`user_permission_read`, `user_role_read`, `role_permission_read`) that already reveal the same facts, because answering it by hand takes many calls and relations, and one grant is what an operator can reason about. Wrapping the policy in a memory-provider `PermissionEvaluator` so both routes share one path was rejected: it overwrites a caller's `permissionBinding` and turns `PolicyError` into `PermissionError`. Pinned by the *(#3604)* cases in `test/unit/http/controllers/entities/{permission,policy}/checker.spec.ts` (a scope-restricted bearer, a subject answered for its own grants, a caller without the grant refused before the lookup, and on the permission route a name refused and the grant matched against the subject's realm) and in `test/unit/core/identity/permission/checker.spec.ts` (the caller form evaluates on the actor's evaluator).
