...
**The two single-subject check routes read WHO to check from the data's own `identity` key, and never remove it (#3604).** The controllers pass the body through unchanged.

- **No `identity`: the caller.** Both services put the actor's `IdentityPolicyData` into the data, as the batch check does, so the identity a check evaluates is always part of the data. `POST /permissions/:id/check` then evaluates on `actor.permissionEvaluator`, which on a request IS the wrapper above and governs that key under the scopes. `POST /policies/:id/check` runs the policy engine without a permission evaluator, so its controller hands the checker an actor whose identity is `useRequestPolicyIdentity(event)`. Both checkers keep `master`'s signature, `check(idOrName, data, actor, realm)`. A bearer without `global` is therefore the only way a check runs anonymously.
- **An `identity` (`null` included): a subject reference.** `resolveCheckSubject` (`core/identity/permission/checker/subject.ts`) accepts only a user or client `{ type, id }` by UUID (400 otherwise; names repeat across realms, every realm has an `admin-console` client), pre-evaluates `PERMISSION_CHECK` (403) BEFORE looking the subject up, so a caller holding no grant learns nothing about which subjects exist, loads it through the identity resolver (404), and evaluates `PERMISSION_CHECK` with `realmMatch` set to the subject's STORED realm, so a `realm_admin` checks subjects of its own realm only and a realm the caller writes is never consulted. The service then replaces the reference in its own copy of the data with the subject as stored, and on the permission route evaluates on a bare `PermissionEvaluator`, since the wrapper would put the caller back. Being refused the subject throws from `safeCheck` rather than answering `status: "error"`, since it answers the request, not the check.
