...
**The two single-subject check routes read WHO to check from the data's own `identity` key, and never remove it (#3604).** Both controllers pass the body through unchanged, with an actor whose identity is `useRequestPolicyIdentity(event)`, and both checkers keep `master`'s signature `check(idOrName, data, actor, realm)`. `buildCheckData` (`core/identity/permission/checker/data.ts`) turns the key into the data both routes evaluate, on a plain evaluator (the permission route) or engine (the policy route), since the data already says who is evaluated:
