...
- **App origins** come from `getAppOrigins(config)` = `[publicUrl, ...trustedOrigins]`
  reduced to bare origins. An `trustedOrigins` entry may carry a scheme
  ...
  list (expansion + dedupe) at config time, so `Config['trustedOrigins']`
