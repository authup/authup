...
`--credential-store=file` / `AUTHUP_CREDENTIAL_STORE=file` explicitly selects
private atomic token files; failures never silently downgrade storage.
One command per server prevents rotation races; SIGINT/SIGTERM releases its
lock. Targets come from `--server` / `AUTHUP_SERVER_URL` (default loopback port
3000), never the deployment document. `healthcheck` and `config validate`
require no login. API paths stay under the configured base URL; redirects
and automatic mutation replay are refused.
