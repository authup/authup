...
- **Field authorization — gated columns (#3322)** — a schema may gate
  ...
  it reaches exactly the readers a plaintext reaches and a read never
...
- **No `identity`: the requester.** The actor's `IdentityPolicyData` goes into the data. A token without `global` hands over no identity, so its check runs without one.
- **`identity: null`: no identity at all**, as the caller asked.
- **An identity: that identity, as given.** When it is not the actor's own (by `type` and `id`), the actor must hold `PERMISSION_CHECK`: pre-evaluated first (403), so a caller holding no grant learns nothing about which subjects exist, then the subject is looked up (404) and the permission evaluated for the actor with only its `identity` and the subject's STORED realm under `realmMatch`. The check data belongs to the permission or policy being checked, so none of it reaches the gate, and the gate carries no `attributes`. The realm comes from the lookup because the grant lookup loads by `id` alone, so a realm written into the data could not bound the gate; a `realm_admin` therefore checks subjects of its own realm only. A foreign identity is named by UUID (400 otherwise), since names repeat across realms. A token without `global` never names its own identity, so naming one always needs the gate. Being refused throws from `safeCheck` rather than answering `status: "error"`, since it answers the request, not the check.
- **The resolved permission row is evaluated**, with its `realmId` and `clientId` on the evaluation context, not a global permission of the same name.

The gate exists because the permission-binding evaluator loads the grants of whatever identity it is handed, and on the policy route the caller also supplies `permissionBinding`, so an ungated subject would let any authenticated caller read anyone's authorization. It is a dedicated permission rather than the grant-read permissions (`user_permission_read`, `user_role_read`, `role_permission_read`) that already reveal the same facts, because answering it by hand takes many calls and relations, and one grant is what an operator can reason about. Wrapping the policy in a memory-provider `PermissionEvaluator` so both routes share one path was rejected: it overwrites a caller's `permissionBinding` and turns `PolicyError` into `PermissionError`. Pinned by the *(#3604)* cases in `test/unit/http/controllers/entities/{permission,policy}/checker.spec.ts` (a scope-restricted bearer, a subject answered for its own grants, a caller without the grant refused before the lookup but answered for its own identity, `null` evaluated without an identity, and on the permission route a name refused, the grant matched against the subject's stored realm and not a realm in the data, and a realm-scoped row evaluated) and in `test/unit/core/identity/{permission,policy}/checker.spec.ts` (the requester default, `null`, and the data reference left untouched).
...
- **Serving seam**: the Vue app ships as
  ...
  workspace symlink and a published install alike; both take the substituted
  `distPath` as an argument and hold no state, and a dist resolves only when
  `dist/client/index.html` AND `dist/server/server.js` exist). It reads the
  built dist (`dist/client/index.html` + `.vite/ssr-manifest.json`,
...
**Critical rule: separate read-path vs write-path EA loading.** `EntityRepositoryAdapter` enforces it (see *Adapter Implementation*): an EA adapter overrides `extendOne` / `extendMany`, which the base calls from `findOneById`, `findOneByName` and `findMany` but never from `findOneBy`, the read a write loads through. `saveWithEA()` does not extend after the save either. It writes the parent row and its attribute rows in ONE transaction (issue #3668), so a failing row write rolls the parent back; it joins a transaction already open on its manager and otherwise opens one on that manager, never a second pooled connection (#3526, #3539). On better-sqlite3 it opens none, since the driver shares one query runner and a transaction there would nest inside whatever else is running (the `isDatabaseTypeRowLockable` rule every transaction seam follows). Pinned on mysql/postgres by `test/unit/adapters/database/extra-attribute-transaction.spec.ts` (sqlite does not enforce the varchar length the spec fails on).
...
field rename, so the booleans stay for this window. **All three modes
ship**: plain and hashed with PR 1, encrypted with PR 2 over the realm
cipher (see *Realm Key Store*), so an encrypted value is the same
`v1.<key_id>.<payload>` AES-256-GCM blob under the client realm's enc key
that protects the MFA seeds. Before PR 2 the flag encrypted nothing, and a
flag nothing honours widened disclosure (#3351). The two flags are
exclusive: `assertSecretModeExclusive` (`core/entities/client/service.ts`)
runs in `save()` over the validated input and in the provisioning
synchronizer before its save, and the endpoint takes ONE `mode`, so it cannot
spell both. A row claiming both would verify under neither, so the
combination answers 400.
...
- **`POST /clients/:id/secret` is the rotation endpoint** and the ONLY
  ...
  keeps the current one as `getClientSecretMode` reads it, and `plain` is
  a valid target. That default is what turns a LEGACY row around: a client
  whose `secretEncrypted` flag was set before the mode existed holds a
  plaintext, and an empty-body rotation of it lands in encrypted mode, the
  flag's stated intent, with a value that is a blob at last. Response 200
  `ClientSecretRotateResponse` = `{ data: Client, meta: { secret } }`: the
  record as stored (the bcrypt hash in hashed mode, the cipher blob in
  encrypted mode) and the plaintext exactly once under `meta.secret`, the
  MFA-enroll precedent for shown-once material.
  `client.client.rotateSecret(id, payload?)` in core-http-kit. A client
  whose `authMethod` is not `secret` answers 400: nothing to rotate.
...
- **`secret` on create and update is always a plaintext.** Create protects
  ...
  client, the bcrypt hash for a hashed one, the cipher blob for an
  encrypted one (the plaintext of the last is read back through the reveal
  rule below). Update accepts `secret` only while the client is plain and
  stores it plain; on a hashed or encrypted client an update carrying
  `secret` answers 400 naming the endpoint, a `null` included (the save
  path would otherwise generate a replacement nobody is told about), so the
  metadata path can never downgrade a protected secret. Switching
...
- **`protect()` does not sniff its input, and `verify()` decrypts.**
  ...
  `{ cipher?: IRealmCipher }` and throws when that mode is asked for with
  no cipher wired or no realm) and returns the input in plain mode. The
  former `isBCryptHash(input) ? input : hash(input)` existed because the
  console posts its whole form back, and it stored a caller-chosen secret
  that happened to look like a bcrypt hash raw, where it could never
  verify; a lookalike plaintext is hashed like any other now.
  `verify(input, client)` mirrors it: hashed is `compare`; encrypted AND
  `isRealmCipherBlob(secret)` is decrypt then a constant-time compare, a
  `RealmCipherBlobError` (an unknown, disabled or foreign key, a payload
  failing GCM authentication) failing the credential CLOSED and anything
  else surfacing as the infrastructure fault it is; plain is the
  constant-time compare. `isRealmCipherBlob` (`core/key/realm-cipher.ts`, a
  `v1.` prefix test) is the ONE discriminator every consumer uses, so a
  legacy row whose flag was set before the mode existed holds a plaintext
  and compares as plain; a bcrypt hash starts with `$2`, a blob with `v1.`,
  ...
  per key id, never the key's status). **The one sniff lives in
  provisioning** (`ClientProvisioningSynchronizer.protectSecret`): a file
  declaring `secretHashed: true` may carry a plaintext or a bcrypt hash, one
  declaring `secretEncrypted: true` a plaintext or a cipher blob, since
  GitOps should not have to hold plaintext, so a raw value is hashed or
...
- **The realm cipher is one DI token, `OAuth2InjectionToken.RealmCipher`**
  ...
  client controller factory (the service's `cipher` ctx), the
  `OAuth2ClientAuthenticator` context behind `/token`, the authorization
  middleware's `ClientAuthenticator` behind Basic auth, and the provisioning
  module for its client synchronizer (falling back to constructing one over
  its own key store in a minimal module graph that never registered the
  token). One instance, because its per-key cipher cache is only useful
  when there is one of it. A `ClientCredentialsService` built without a
  cipher refuses to protect in encrypted mode and verifies a blob as false.
- **An encrypted secret is tied to the enc key's lifecycle.**
  `KeyRepositoryAdapter.countBlobReferences` counts `auth_clients.secret`
  next to the MFA seeds (`LIKE 'v1.<key_id>.%'` over both tables), so
  `DELETE /keys/:id` on an enc key holding client secrets answers 409 with
  the count and `?force=true` crypto-shreds them: such a client can no
  longer authenticate afterwards (the decrypt fails closed) and needs a
  rotation. Disabling the key is the reversible kill switch and stops every
  client encrypted under it from authenticating until it is enabled again;
  a passive key keeps decrypting, so a rotation is no sweep. That blast
  radius is why the mode stays opt-in per client, why the kit's dialog says
  so, and why a production deployment should set `SECRETS_ENCRYPTION_KEY`:
  without the KEK the enc key's material sits unwrapped in `auth_keys`, and
  a column encrypted under it in the same database is cohort parity, not a
...
- **`auth_clients.secret` is `varchar(512)`** (entity `length: 512`): a
  ...
  length differs, so `migration generate` emits a `DROP COLUMN` that empties
  every client secret, `system` included (see
  `.agents/references/typeorm.md`). `down()` narrows back to 256 and fails
  when a row holds a longer value, which is exactly an encrypted secret:
  rotate such clients to another mode before reverting.
- **The write lock-reads the row; the protection happens before it.**
  `rotateSecret` runs `protect()` BEFORE the transaction (hashing is CPU,
  encryption reads `auth_keys`, and neither may run on a second pooled
  connection while the first is pinned `FOR UPDATE`: the #3526 / #3539 pool
  deadlock, invisible on sqlite), then writes inside the same
  `repository.transaction` seam `save()` uses. The fresh row is lock-read,
  `authMethod` is re-checked against it and the three columns are set on
  it. The re-check is the PR 1 review fix: a concurrent switch to `none`
  landing between the unlocked read and the lock is refused with 400 and
  the cleared credential stays cleared, where saving the entity read above
  would have had TypeORM diff it against the fresh row and write
  `authMethod: 'secret'` back. The update path stays plain-only for the pool
  reason: its own `protect()` runs inside the callback and must never need
  the cipher there.
...
encrypted mode choice and a show-once panel with a copy button; a hint under
the encrypted option says the value is stored under the realm's encryption
key, can be shown again to a permitted reader, and that disabling or
deleting that key stops such clients from authenticating. The dialog opens
on the entity's current mode, so a legacy `secretEncrypted` client defaults
to encrypted, and the rotated entity is adopted the way an update's response
is.
...
rotate to encrypted and create in encrypted mode, each authenticating at
`/token` and revealed to an admin on the single and the list read; hashed
and encrypted at once refused; a public client refused; self-rotation via
...
under `secretHashed: true` stored as bcrypt, a bcrypt value kept verbatim, a
raw value re-hashed on merge, a plain value verbatim, a raw value under
...
- **The shell route is a wildcard.** The console's routes nest
  ...
  (`adapters/http/constants.ts`, next to `ACCOUNT_CONSOLE_SEGMENT`; the two
  ...
  name carries a content hash; a new build means new names), for all three
...
- **Route meta replaces `definePageMeta`.** `src/router.ts` is an explicit
  ...
  the `vite:preloadError` event with a full load of the target
  (`src/chunk-recovery.ts`, `installChunkLoadRecovery`). The load is
...
**The boot-time assert lives in the auth console service now**
(`assertRenderContract` in `apps/server-auth-console/src/render.ts`, run by
`createHandler`, which IS the service's boot). It `import()`s
`dist/server/server.js` and refuses to start unless the bundle's exported
...
actionable error, never a failed boot; `resolveDistPath` requires
...
flips, and that sibling reports its own instant. The end is reported as the end plus 60 s of
elapsed time, never as the local minute after it: on a DST fall-back day an end just
before the repeated hour would otherwise name its second occurrence, an hour after the
real flip. `buildAuthorizationCheck` hands every (name, realm) pair a collector of its own
(`createPolicyTransitionCollector()`) and keeps a pair's instant only when the pair's
clock can move THIS caller's answer: the pair is held; or it is denied and the caller has
an identity holding a grant for that permission (read through the same memoized grant
load, so a failed load is re-raised as everywhere else); or it is denied and its
definition does not deny every grant-less caller. `deniesWithoutGrant` decides the last:
a non-inverted binding node reached only through non-inverted composites that cannot pass
without it (UNANIMOUS: any child; AFFIRMATIVE: every child; the definition's policies
combined under its own `decisionStrategy`), anything else answering "may pass" so that no
transition that could matter is dropped. A pair denied for want of a grant has a window
that would only disclose the boundaries of a permission the caller cannot hold (an
anonymous caller included) and have it refetch at them for nothing; keeping it also made
the answer depend on the unordered order of a definition's policies, since a
`system.default` that settles first short-circuits the clock policy. The result is
...
another commit), and when the memo no longer holds the answer it was built from: a
resolve with other grants replaced it, and that load commits its own, newer verdicts. A
failed timer refetch logs nobody out: it keeps the old evaluator and retries after 30 s,
doubling up to five minutes and starting over after a success. A cookie-mode resolve
revalidates anyway and refetches an expired memo on the way; a validated BEARER session
is not revalidated per navigation, so its resolve refetches an expired memo explicitly
through the same never-throwing path, which is what catches a timer that sleep or
background throttling held back (#3618). The store's `permissionRevision` ref, which
`usePermissionCheck`'s watch and the admin console's sidebar key on next to `status`,
is bumped when a swap or a commit of a session already committed installs verdicts that
DIFFER from the committed ones, compared in a canonical form (the identity's id and
realm, the verdicts sorted, or the introspection's grants for the name-only view). An
...
belongs to the dev CLI, not to `authup start`). The helper's vocabulary is
`run|revert|status` unless the caller opts in with
`defineCLIMigrationCommand(configFs, { generate: true })`, which only
server-core's dev CLI does: `generate` drops and recreates the localhost
compose databases and writes into the checkout, so the operator binary refuses
it as an unknown operation before anything is touched (pinned in
`apps/server-core/test/unit/cli/module.spec.ts`).
...
handler someone else owns**: each exports `createApplication` (aliased
`create<Name>ConsoleApplication` only at `apps/authup`'s import site) over
the kit's own `createApplication` in `@authup/server-console-kit`, which is
the whole of its lifecycle (resolve the config, load the theme, build the
handler, listen, close). That is what `authup start console` starts and what
the `authup-<name>-console` bin starts (`createApplication().setup()` plus a
SIGINT/SIGTERM teardown pair of its own, since a console cannot import
server-core's `registerShutdownHandlers`), so the two paths cannot diverge,
and server-core neither imports nor mounts any of it. `createServer` survives
as an exported bare-handler factory with no caller in the repo.
...
`buildSchemaJSONSchema`. Each console service's `resolveConfig` runs the same
`mountSchema` pass over its OWN selection (a validup `Container.run`, which
is why `resolveConfig` / `readConfigFromEnv` are async) before layering its
defaults, so a document key only a console reads is refused at boot exactly
as `authup config validate` refuses it. The file read is what makes that
...
one folder per section under `src/sections/`: `root/` (the root keys),
`theme/`, `core/` (`core.*`) and one per console (`<name>-console/`, the
`<name>Console.*` keys). A service does not declare anything.
...
the root section plus core plus the five console-reference keys it needs to
...
file loader and exiting 1. Boot and validate agree: every service, the
three consoles included, runs the same mounted zod types over the keys it
reads (see *Six passes* above). server-core keeps the pieces its own dev CLI
...
- The auth console (`@authup/server-auth-console`) takes an optional
  `render` on `createHandler`, matching the `RenderPage` signature
  `createRenderPage(distPath)` returns. Dev's
...
wildcard, no comma list, no userinfo; `null` = no push) is the endpoint an RP
registers to be told when a session it received tokens for ends. The
validator's `.max(2000)` mirrors the column on purpose: without it an over-long
value passed validation and failed at the driver, which `sanitizeError` maps to
`INTERNAL_ERROR`, so the field answered 500 where every capped sibling
(`displayName`, `secret`, `grantTypes`) answers 400. `baseUrl` still carries
that gap (pre-existing, `z.url().nullable()` with no cap). It is the push half of the `sid` claim:
...
the same reason. The completion therefore rides `isSameOriginRequest`, the
same predicate every cookie-authenticated surface does: `Sec-Fetch-Site:
same-origin` is mandatory and fails closed when absent, and `Origin`, which a
browser sends on every POST, must be publicUrl's own. An Origin-less POST used
...
- **Forward-only for the name, verification-aware for the email.** The
  ...
  re-read through `IUserIdentityRepository.findOneById`, which re-selects
  the `select: false` column, because `account.user` is loaded under the
...
ends every encrypted client's authentication in that realm until it is
enabled again, and a force delete ends it for good (see *Client secret
storage and rotation*). That is one more reason to set
`SECRETS_ENCRYPTION_KEY` in production: without the KEK the enc key material
sits unwrapped in `auth_keys`, and a column encrypted under it in the same
database is cohort parity, not a defence against a database read.
...
- **Per-request isolation is the security property.** A collection key is
  ...
  payload literal, and the memos `createRenderPage` keeps in its per-handler
  closure hold only the immutable template / manifest / bundle), and the store is provided on
