...
- **`save` answers a unique-index refusal with `EntityConflictError` (409)**, the losing side of a race `checkUniqueness` cannot close. Recognizable by `isEntityConflictError`, which `ensurePath` and the federated account manager rely on to re-read the winner. `checkUniqueness` keeps throwing `DatabaseConflictError` (same `ENTITY_CONFLICT` code).
