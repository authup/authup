...
tick before its ids on every folder change. A folder in the route also counts
as pending until the `PATH_READ` check has SETTLED (the kit's
`usePermissionCheckState`, whose `settled` flag is what tells a denial from
the fail-closed default), since `path` reads null until then and the page's
first load would otherwise list every row in the realm (#3632). All three are
what make `?path=` reach the server at all; the folder scope hit each of them.
...
notice instead of what reads as an empty folder. The pane's own lookup carries
