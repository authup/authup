...
|-------------------------------------------|-------------|-------------------------------------------------------------------------------------------------------|
| [authup](../apps/authup)                  | CLI         | The operator CLI and the ecosystem's only binary (`authup`). It imports `@authup/server-core` and runs it **in process**: `start`, `worker`, `migration`, `healthcheck` and `config` (`validate` / `schema`, plan 101 C-2) compose the `defineCLI*Command` helpers server-core exports, so the command bodies stay with the service and a downstream embedder can reuse them. No spawn, no supervisor, no config forcing (plan 101 D1). The process the operator starts IS the server, so signals, the exit code and the environment reach it directly, and `PORT`/`HOST` follow the ordinary precedence (environment beats the configuration file, one `authup.yml`). Root args `--configDirectory` / `--configFile` reach every subcommand, nested ones included (citty runs the root `setup` before it recurses); a stray positional on `start` / `worker` (the retired `authup start server.core` selector shape) is refused. |
...
Apps:
  ...
  authup (CLI)      → server-core
                      (an in-process CLI: it IMPORTS server-core and runs its application factories, so
                       server-core is an ordinary runtime dependency; every console rides server-core, so
                       the CLI resolves nothing else)
```
