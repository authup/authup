...
| **Tables** | `@vuecs/table` (≥ 1.1.1) | `<VCTable>` directly. `:data` + `:columns` (`TableColumn[]`) drives auto-render; consumer-side `#cell-<key>` / `#header-<key>` slot templates are dispatched onto each cell by `composeTableInner` (tada5hi/vuecs#1592). Centered headers use `headerClass: 'text-center!'` — Tailwind v4's `!important` suffix beats theme-tailwind's baked-in `text-left` on `tableHeadCell.classes.root`. Cells need no `!` — `<VCTableCell>` bakes only `align-middle`. |
