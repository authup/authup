...
| [server-core](../apps/server-core)        | Service     | A service that forms the backbone of the server-side ecosystem. Serves the OAuth2 `/authorize` consent page and the auth workflow pages by rendering the `@authup/client-auth-console` SSR bundle per request, and serves the `@authup/client-account-console` and `@authup/client-admin-console` SPA bundles at `/console/account` and `/console/admin`. Its `authup-server` binary carries two more long-running roles next to `start`: `authup-server worker` (plans 095/096/097) runs the background cron sweeps alone, with no HTTP listener and no migrations, so API replicas can hand them over via `componentsEnabled`; `authup-server console [admin\|account]` (plan 099 PR 1b) is the IdP without its management API, serving the consoles, the auth pages, the protocol and everything those pages call back on their replica, while the entity CRUD 404s (`HTTPModule({ managementApi: false })`; the positionals are sugar over the two console flags). See architecture.md → *Deployment Topology*. |
