...
|-------------------------------------------|-------------|-------------------------------------------------------------------------------------------------------|
| [authup](../apps/authup)                  | CLI         | The operator CLI and the one binary an ordinary deployment runs. It imports the service packages and runs them **in process**: `start`, `core`, `worker`, `migration`, `healthcheck` and `config` (`validate` / `schema`, plan 101 C-2) compose the `defineCLI*Command` helpers server-core exports, so the command bodies stay with the service and a downstream embedder can reuse them; `console [admin\|account\|auth]` is the CLI's own (`src/roles/console.ts`), since a console service is a listener rather than a module graph. No spawn, no supervisor, no config forcing (plan 101 D1). The process the operator starts IS the server, so signals, the exit code and the environment reach it directly, and `PORT`/`HOST` follow the ordinary precedence (environment beats the configuration file, one `authup.yml`). **It is also the only place that knows about all four config registries**: `src/roles/config.ts` holds server-core's plus the three console services', hands each service its own resolved section (`readConsoleConfigs`, resolving path keys against server-core's `rootPath` so one document means the same thing to every service it configures), and passes the console registries into `config validate` / `config schema` so a console's key is neither reported as unread nor missing from the emitted JSON Schema. Root args `--configDirectory` / `--configFile` reach every subcommand, nested ones included (citty runs the root `setup` before it recurses); a stray positional on `core` / `start` / `worker` (the retired `authup start server.core` selector shape) is refused. |
...
Layer 1:
  server-config-base → kit, server-config-kit (the keys more than one server
                      package reads, declared once; nothing may import across
                      the service boundary in either direction)
  specs             → kit, errors
...
Apps:
  ...
  server-core       → access, i18n, kit, core-kit, core-http-kit, errors, server-config-kit,
                      ...
                      (NO console package is a dependency any more: the three client-*-console dists,
                       ...
                       is ~28 MB of dist an `authup core` deployment never installs. server-console-kit
  ...
  server-auth-console → client-auth-console (RUNTIME: the SSR bundle it renders), core-http-kit (it
                      ...
                      (NOT server-core, in any of the three: a console reaching into it would drag the
                       ...
                       deployment. The keys they share are declared once in server-config-base)
