...
| [errors](../packages/errors)                    | Library     | `AuthupError` (extends `BaseError` from `@ebec/core`), error-code constants, built-in subclasses (`BadRequestError`, `EntityNotFoundError`, ...), code→HTTP-status mapping, and `Symbol.for(...)`-keyed duck guards. `AuthupError.toJSON()` emits the `@instanceof` marker chain as a string list, and every duck guard's fast path is `matchesInstanceof` (symbol **or** serialized-string chain) — so guards keep the inheritance match for JSON-rehydrated errors (#3042); new guard modules must use `matchesInstanceof` from `@authup/errors`, never raw `hasInstanceof`. |
