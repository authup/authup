...
|-------------------------------------------|-------------|-------------------------------------------------------------------------------------------------------|
| [authup](../apps/authup)                  | CLI         | The operator CLI and the ecosystem's only binary (`authup`). It imports `@authup/server-core` and runs it **in process**: `start`, `worker`, `migration` and `healthcheck` compose the `defineCLI*Command` helpers server-core exports, so the command bodies stay with the service and a downstream embedder can reuse them. No spawn, no supervisor, no config forcing (plan 101 D1). The process the operator starts IS the server, so signals, the exit code and the environment reach it directly, and `PORT`/`HOST` follow the ordinary precedence (environment beats the configuration file). Root args `--configDirectory` / `--configFile` reach every subcommand; a stray positional on `start` / `worker` (the retired `authup start server.core` selector shape) is refused. |
