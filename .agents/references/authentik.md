...
| `AuthenticatedSession` (`authentik/core/models.py`): `OneToOneField(Session, primary_key=True)` + `user` FK. **One per browser**, with no application/provider field. Per-app identity hangs on the TOKEN: `BaseGrantModel.session = FK(AuthenticatedSession)` on `AuthorizationCode` / `AccessToken` / `RefreshToken` (`authentik/providers/oauth2/models.py`) | `auth_sessions` (one row per browser, `client_id` = the client that FIRST authorized on it) + `auth_session_tokens.client_id` (per-token attribution) | **Aligned since plan 086.** Before it, `resolveSession` reused a session across a different client and overwrote `existing.clientId`, so per-app attribution was last-writer-wins for every app sharing the IdP origin, and `auth_session_tokens` carried no client column at all. Authentik's browser session is deliberately client-agnostic and the provider link rides each token row; Keycloak agrees on the principle with a different shape (`UserSessionModel` with many `AuthenticatedClientSessionModel` children, keyed by client UUID). authup now matches the Authentik shape: one session row, attribution one level down. Note Authentik CASCADEs `AccessToken`/`AuthorizationCode` on session delete but uses `SET_DEFAULT` for `RefreshToken` (it survives logout), whereas authup CASCADEs every token row. Verified against `main` 2026-08-04. |
