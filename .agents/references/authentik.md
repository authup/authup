...
| Reputation policy (±5 bounds), GeoIP + impossible travel (2025.2), lockout | Global identity-aware rate-limit middleware only; dead `AttemptLogin` type | Plan 053: throttle over plan 057's `audit_events`; reputation deferred; GeoIP/impossible-travel rejected (plan 062). |
| Events + notification rules (transports; 365d retention default) | Winston logger; redis/socket domain events (CRUD only — auth paths emit nothing) | Plan 057: persisted `audit_events`, own storage, 365d default matches; notification-rules engine rejected (plan 062). |
