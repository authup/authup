...
| Events + notification rules (transports; 365d retention default) | Winston logger; redis/socket domain events (CRUD only — auth paths emit nothing) | Plan 057: persisted `auth_events`, own storage, 365d default matches; notification-rules engine rejected (plan 062). |
...
| RBAC: roles as sole permission carrier (2025.12), group hierarchy | Roles + direct user/client/robot permission junctions with `realm_scope` + per-grant policy | authup keeps direct identity grants; groups hierarchy rejected (plan 062). |
...
| Secrets at rest (verified 2026-07-14): TOTP device `key` is a **plaintext** hex CharField (django-otp `TOTPDevice.key`); OAuth2/SAML signing private keys live **plaintext** in the DB `certificates` table (leaked via unauthenticated API in CVE-2024-42490) | TOTP seeds AES-256-GCM-encrypted under env `MFA_ENCRYPTION_KEY` (`SymmetricCipher`); recovery codes bcrypt; **but** `auth_keys.decryption_key` (realm JWT private key) is plaintext base64, `select:false` only | authup is **stricter than Authentik on MFA seeds**, at parity (plaintext) on realm signing keys. Per-realm DB-stored MFA keys discussed 2026-07-14 → rejected unless envelope-wrapped (KEK stays in env/vault); see also `.agents/references/keycloak.md`. |
