...
- **`POST /token/introspect` READS an expired token and reports it; it raises only for a token it cannot read.** The verify passes `ignoreExpiry`, so an expired token still yields its payload and the subject's claims, and the response is `200 {active: false, ...payload, ...claims}` where it used to be a thrown `401`. That is the point: a third-party app can say "your session ended, \<name\>" instead of only "no". What still raises is a token that cannot be read at all — malformed, bad signature, a `kid` naming no key — because reporting one as inactive would claim authup issued it and let it lapse; the same goes for a verifying token with no `sub` and for a subject that no longer resolves (`identity_invalid`). **`active` is derived in the CONTROLLER, never from the verify**: `OAuth2TokenVerifier`'s signature-keyed cache returns a hit without re-checking `exp`, so under `ignoreExpiry` the verify alone would report an expired token as live. It is `jti` not blocklisted AND `exp` in the future, fail-closed on either claim missing. The verifier's own rule — never `saveWithSignature` on an `ignoreExpiry` path — is what keeps an expired payload out of that cache in the first place; do not relax it. Note the coupling with `@authup/client-web-kit`: the store's `revalidate()` relied on introspection *throwing* to reach its refresh fallback, so the store now checks `active` on the response (see architecture.md → *What the kit store persists*) — the two must not be separated.
- **`POST /token/revoke` answers its ordinary `202` for a token it cannot read**, which is the one place an RFC names invalid tokens outright: RFC 7009 §2.2, "invalid tokens do not cause an error response since the client cannot handle such an error in a reasonable way". Expiry was already bypassed via `ignoreExpiry`; the `if (isJWTError(e))` branch covers malformed and unverifiable, the `HTTPOAuth2RefreshTokenGrant` call-site shape. A missing `token` **parameter** is a malformed request and stays `400 invalid_request` here and on introspection alike.
