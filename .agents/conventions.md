...
- **`POST /token/revoke` answers its ordinary success status for a token it cannot read**, which is the one place an RFC names invalid tokens outright: RFC 7009 §2.2, "invalid tokens do not cause an error response since the client cannot handle such an error in a reasonable way". Expiry was already bypassed via `ignoreExpiry`; the `if (isJWTError(e))` branch covers malformed and unverifiable, the `HTTPOAuth2RefreshTokenGrant` call-site shape. Note that §2.2 names **`200`** as that status and authup answers **`202`** — an authup choice predating this rule, not something the RFC asks for; what the RFC asks for is that an invalid token be indistinguishable from a revoked one, which holds. A missing `token` **parameter** is a malformed request and stays `400 invalid_request` here and on introspection alike.
