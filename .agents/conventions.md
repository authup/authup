...
- **Property naming (plan 073)**: new entity/domain properties and management-API payload/response fields must be **camelCase**; the snake_case DB column is derived automatically by the `SnakeNamingStrategy`, so never hand-write a snake_case property. The OAuth2/OIDC protocol surface and JWT claims stay snake_case. See `.agents/architecture.md#naming-split-plan-073`.
