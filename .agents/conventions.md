...
- **The token endpoint's other two verifying handlers follow the same rule, and each RFC names a different answer.** `POST /token/introspect` reports an unverifiable token as `200 {active: false}` (RFC 7662 §2.2 — a token that is not active, does not exist, or cannot be verified is *reported*, not raised; the same goes for a subject that no longer resolves, which used to answer 400). `POST /token/revoke` answers its ordinary `202` (RFC 7009 §2.2 — "invalid tokens do not cause an error response since the client cannot handle such an error in a reasonable way"; expiry was already bypassed via `ignoreExpiry`, this covers malformed and unverifiable). Both are `if (isJWTError(e))` branches in `adapters/http/controllers/workflows/token/module.ts`, the `HTTPOAuth2RefreshTokenGrant` shape. A missing `token` **parameter** is a malformed request and stays `400 invalid_request` on both. Note the coupling this creates with `@authup/client-web-kit`: the store's `revalidate()` used to rely on introspection *throwing* to reach its refresh fallback, so the store checks `active` on the response (see architecture.md → *What the kit store persists*) — the two must not be separated.
