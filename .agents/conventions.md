...
- **Neither `/token/introspect` nor `/token/revoke` requires authorization today, and RFC 7662 §2.1 makes that a MUST** ("To prevent token scanning attacks, the endpoint MUST also require some form of authorization"). Closing it is not a middleware line: the kit store introspects with `Bearer <the token being introspected>`, so a bearer gate would reject the expired token the endpoint exists to report, and a client-credentials gate would lock out the secret-less console clients. Tracked in issue #3489 — do not add an anonymous read to either route in the meantime.
