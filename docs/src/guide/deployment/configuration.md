...
Every CLI command (`start`, `worker`, `console`, `migration`, `healthcheck`) honors them, and the lookup
...
- **Multi-section file** (`authup.conf`): one file for the whole stack, with options
  namespaced per component (`server.core.*`). `server/core` reads its own section
  out of it.
