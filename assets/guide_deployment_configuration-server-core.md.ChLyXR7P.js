import{_ as i,o as a,c as n,a0 as l}from"./chunks/framework.ZNXClGez.js";const c=JSON.parse('{"title":"Configuration","description":"","frontmatter":{},"headers":[],"relativePath":"guide/deployment/configuration-server-core.md","filePath":"guide/deployment/configuration-server-core.md"}'),e={name:"guide/deployment/configuration-server-core.md"};function p(t,s,h,k,r,d){return a(),n("div",null,[...s[0]||(s[0]=[l(`<h1 id="configuration" tabindex="-1">Configuration <a class="header-anchor" href="#configuration" aria-label="Permalink to &quot;Configuration&quot;">​</a></h1><p>The API configuration is read from the <code>server.core</code> section of <code>authup.yml</code> (or of <code>authup.{json,js,ts,...}</code>; see the <a href="./configuration.html">configuration introduction</a> for the file lookup and the <code>--configDirectory</code>/<code>--configFile</code> CLI flags).</p><p>Most options can also be provided as environment variables (shown in the <code>.env</code> tab below). <strong>An environment variable always overrides the file value</strong> for the same option. A few options are file-only and have no environment variable — they are marked as such.</p><p>Not every option below sits under <code>server.core</code>. <code>env</code>, <code>host</code>, <code>rootPath</code>, <code>publicUrl</code> and <code>trustedOrigins</code> are top-level, the two theme options are <code>theme.directoryPath</code> and <code>theme.fragmentsEnabled</code>, and the console options are <code>server.adminConsole.*</code>, <code>server.accountConsole.*</code> and <code>server.authConsole.*</code>. The <code>authup.yml</code> tab shows each one at its place.</p><p>Of those, server-core itself reads only <code>enabled</code> and <code>url</code> per console: <code>enabled</code> gates the console&#39;s two sign-in routes, <code>url</code> is where it sends the browser. The rest of a console&#39;s section, and the whole <code>theme</code> section, are read by the <strong>console service</strong> that serves the console, so setting them on an API-only process (<code>authup core</code>) does nothing. See <a href="./console-replicas.html">Console Replicas</a> for the full per-console option list.</p><div class="danger custom-block"><p class="custom-block-title">Security</p><p>The admin password and the <code>system</code> client secret both default to <code>start123</code>. Set your own values (<code>userAdminPassword</code> / <code>clientSystemSecret</code>, or the <code>USER_ADMIN_PASSWORD</code> / <code>CLIENT_SYSTEM_SECRET</code> environment variables) before deploying to production — the examples below carry placeholders, never copy a credential out of documentation.</p></div><p>For MFA enforcement behavior and its federated-login, password-grant, feature-toggle, and cache-availability boundaries, see <a href="./configuration-server-core-mfa.html">Multi-factor authentication</a>.</p><p>The infrastructure connections are documented on their own pages: <a href="./configuration-server-core-database.html">Database</a> (<code>db</code>), <a href="./configuration-server-core-redis.html">Redis</a> (<code>redis</code>) and <a href="./configuration-server-core-smtp.html">SMTP</a> (<code>smtp</code>).</p><div class="vp-code-group vp-adaptive-theme"><div class="tabs"><input type="radio" name="group-m1C08" id="tab-uR5NX23" checked><label data-title="authup.ts" for="tab-uR5NX23">authup.ts</label><input type="radio" name="group-m1C08" id="tab-Cr7BqNI"><label data-title="authup.yml" for="tab-Cr7BqNI">authup.yml</label><input type="radio" name="group-m1C08" id="tab-sJTYr4M"><label data-title=".env" for="tab-sJTYr4M">.env</label></div><div class="blocks"><div class="language-typescript vp-adaptive-theme active"><button title="Copy Code" class="copy"></button><span class="lang">typescript</span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"></span>
...
<span class="line"><span style="--shiki-light:#6A737D;--shiki-dark:#6A737D;">     * publicUrl origin. In non-production, the admin console&#39;s standalone</span></span>
<span class="line"><span style="--shiki-light:#6A737D;--shiki-dark:#6A737D;">     * vite dev server origin (http://localhost:3010) is seeded</span></span>
<span class="line"><span style="--shiki-light:#6A737D;--shiki-dark:#6A737D;">     * automatically.</span></span>
...
<span class="line"><span style="--shiki-light:#6A737D;--shiki-dark:#6A737D;">             * listener, used by \`authup console\`) are documented in the</span></span>
...
<span class="line"><span style="--shiki-light:#6A737D;--shiki-dark:#6A737D;">            /**</span></span>
<span class="line"><span style="--shiki-light:#6A737D;--shiki-dark:#6A737D;">             * Directory the application writes to at runtime (log files in</span></span>
<span class="line"><span style="--shiki-light:#6A737D;--shiki-dark:#6A737D;">             * production) and reads file-based provisioning from</span></span>
<span class="line"><span style="--shiki-light:#6A737D;--shiki-dark:#6A737D;">             * (&lt;writableDirectoryPath&gt;/provisioning). Relative paths are resolved</span></span>
...
<span class="line"></span>
<span class="line"><span style="--shiki-light:#6A737D;--shiki-dark:#6A737D;">            /**</span></span>
<span class="line"><span style="--shiki-light:#6A737D;--shiki-dark:#6A737D;">             * Run the background components (the session, token and audit-event</span></span>
<span class="line"><span style="--shiki-light:#6A737D;--shiki-dark:#6A737D;">             * sweeps) in this process. Set false on API replicas once a dedicated</span></span>
<span class="line"><span style="--shiki-light:#6A737D;--shiki-dark:#6A737D;">             * worker process runs them, and only then: with it false everywhere,</span></span>
<span class="line"><span style="--shiki-light:#6A737D;--shiki-dark:#6A737D;">             * nothing sweeps. A worker started with \`authup worker\` forces</span></span>
<span class="line"><span style="--shiki-light:#6A737D;--shiki-dark:#6A737D;">             * them on regardless of this option. See the Worker guide.</span></span>
<span class="line"><span style="--shiki-light:#6A737D;--shiki-dark:#6A737D;">             * env: COMPONENTS_ENABLED</span></span>
<span class="line"><span style="--shiki-light:#6A737D;--shiki-dark:#6A737D;">             * default: true</span></span>
<span class="line"><span style="--shiki-light:#6A737D;--shiki-dark:#6A737D;">             */</span></span>
<span class="line"><span style="--shiki-light:#24292E;--shiki-dark:#E1E4E8;">            componentsEnabled: </span><span style="--shiki-light:#005CC5;--shiki-dark:#79B8FF;">true</span><span style="--shiki-light:#24292E;--shiki-dark:#E1E4E8;">,</span></span>
<span class="line"></span>
<span class="line"><span style="--shiki-light:#6A737D;--shiki-dark:#6A737D;">            /**</span></span>
...
<span class="line"><span style="--shiki-light:#22863A;--shiki-dark:#85E89D;">    writableDirectoryPath</span><span style="--shiki-light:#24292E;--shiki-dark:#E1E4E8;">: </span><span style="--shiki-light:#032F62;--shiki-dark:#9ECBFF;">writable</span></span>
<span class="line"><span style="--shiki-light:#22863A;--shiki-dark:#85E89D;">    componentsEnabled</span><span style="--shiki-light:#24292E;--shiki-dark:#E1E4E8;">: </span><span style="--shiki-light:#005CC5;--shiki-dark:#79B8FF;">true</span></span>
...
<span class="line"><span style="--shiki-light:#E36209;--shiki-dark:#FFAB70;">COMPONENTS_ENABLED</span><span style="--shiki-light:#D73A49;--shiki-dark:#F97583;">=</span><span style="--shiki-light:#24292E;--shiki-dark:#E1E4E8;">true</span></span>
