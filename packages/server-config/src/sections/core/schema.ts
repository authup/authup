...
export const CORE_SCHEMA = defineSchema<CoreConfig, never, EnvironmentVariable>(
    {
        ...
        querySchemaDiscoveryEnabled: {
            type: booleanType,
            default: true,
            description: 'Serve the query-schema discovery routes (GET /schemas and GET /<collection>/@schema), which report the queryable vocabulary of every registered entity to an authenticated caller. Disabled, both answer 404 to an authenticated caller; an anonymous one is refused with 401 either way, since the login gate runs first.',
