...
export class PolicyAPI extends BaseAPI implements DomainAPI<Policy> {
    async getMany(data?: BuildInput<Policy>): Promise<CollectionResourceResponse<Policy>> {
        const response = await this.client.get(`policies${buildQuery(data)}`);
        return response.data;
    }

    async delete(id: Policy['id']): Promise<SingleResourceResponse<Policy>> {
        const response = await this.client.delete(`policies/${id}`);

        return response.data;
    }

    async getOne(id: Policy['id'], record?: BuildInput<Policy>) {
        const response = await this.client.get(`policies/${id}${buildQuery(record)}`);

        return response.data;
    }

    async create(data: Partial<Policy>): Promise<SingleResourceResponse<Policy>> {
        const response = await this.client.post('policies', nullifyEmptyObjectProperties(data));

        return response.data;
    }

    async update(id: Policy['id'], data: Partial<Policy>): Promise<SingleResourceResponse<Policy>> {
