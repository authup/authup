...
export class RoleAPI extends BaseAPI implements IRoleAPI {
    ...
    async getOne(roleId: Role['id'], record?: EntityQueryInput<Role>): Promise<EntityRecordWrappedResponse<Role>> {
        const response = await this.client.get(`roles/${roleId}${buildQueryString(record)}`);

        return response.data;
    }

    async delete(roleId: Role['id']): Promise<EntityRecordWrappedResponse<Role>> {
        const response = await this.client.delete(`roles/${roleId}`);

        return response.data;
    }

    async create(data: RoleCreatePayload): Promise<EntityRecordWrappedResponse<Role>> {
        const response = await this.client.post('roles', nullifyEmptyObjectProperties(data));

        return response.data;
    }

    async update(id: Role['id'], data: RoleUpdatePayload): Promise<EntityRecordWrappedResponse<Role>> {
