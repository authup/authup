...
export class IdentityProviderAPI extends BaseAPI implements IIdentityProviderAPI {
    ...
    async delete(id: IdentityProvider['id']): Promise<EntityRecordWrappedResponse<IdentityProvider>> {
        const response = await this.client.delete(`identity-providers/${id}`);

        return response.data;
    }

    async create(data: IdentityProviderCreatePayload): Promise<EntityRecordWrappedResponse<IdentityProvider>> {
        const response = await this.client.post('identity-providers', nullifyEmptyObjectProperties(data));

        return response.data;
    }

    async update(id: IdentityProvider['id'], data: IdentityProviderUpdatePayload): Promise<EntityRecordWrappedResponse<IdentityProvider>> {
