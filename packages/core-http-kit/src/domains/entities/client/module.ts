...
export class ClientAPI extends BaseAPI implements IClientAPI {
    async getStats(query: EntityStatsQuery<Client> = {}): Promise<EntityStatsResponse> {
        const response = await this.client.get(buildStatsURL('clients', query));

        return response.data;
    }

    async getSchema(): Promise<EntitySchemaResponse> {
        const response = await this.client.get('clients/@schema');
