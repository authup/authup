...
export class RoleAttributeAPI extends BaseAPI implements IRoleAttributeAPI {
    ...
    async getOne(roleId: RoleAttribute['id'], record?: EntityQueryInput<RoleAttribute>): Promise<EntityRecordWrappedResponse<RoleAttribute>> {
        const response = await this.client.get(`role-attributes/${roleId}${buildQueryString(record)}`);

        return response.data;
    }

    async delete(roleId: RoleAttribute['id']): Promise<EntityRecordWrappedResponse<RoleAttribute>> {
        const response = await this.client.delete(`role-attributes/${roleId}`);

        return response.data;
    }

    async create(data: RoleAttributeCreatePayload): Promise<EntityRecordWrappedResponse<RoleAttribute>> {
        const response = await this.client.post('role-attributes', nullifyEmptyObjectProperties(data));

        return response.data;
    }

    async update(id: RoleAttribute['id'], data: RoleAttributeUpdatePayload): Promise<EntityRecordWrappedResponse<RoleAttribute>> {
