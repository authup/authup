...
export class SessionAPI extends BaseAPI implements ISessionAPI {
    ...
    async getOne(id: Session['id'], record?: EntityQueryInput<Session>): Promise<EntityRecordWrappedResponse<Session>> {
        const response = await this.client.get(`sessions/${id}${buildQueryString(record)}`);

        return response.data;
    }

    async delete(id: Session['id']): Promise<EntityRecordWrappedResponse<Session>> {
