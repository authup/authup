...
export class KeyAPI extends BaseAPI implements IKeyAPI {
    ...
    async getOne(id: Key['id'], record?: EntityQueryInput<Key>): Promise<EntityRecordWrappedResponse<Key>> {
        const response = await this.client.get(`keys/${id}${buildQueryString(record)}`);

        return response.data;
    }

    async create(data: KeyCreatePayload): Promise<EntityRecordWrappedResponse<Key>> {
        const response = await this.client.post('keys', nullifyEmptyObjectProperties(data));

        return response.data;
    }

    async update(id: Key['id'], data: KeyUpdatePayload): Promise<EntityRecordWrappedResponse<Key>> {
        const response = await this.client.post(`keys/${id}`, nullifyEmptyObjectProperties(data));

        return response.data;
    }

    async delete(id: Key['id'], options?: KeyDeleteOptions): Promise<EntityRecordWrappedResponse<Key>> {
