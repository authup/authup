 ...
 * `authup-account-console` bin starts, so the two paths cannot diverge.
 */
export function createAccountConsoleApplication(config: Config) : ConsoleApplication {
    return defineConsoleApplication({
        name: 'account console',
        port: config.port,
        host: config.host,
        createHandler: () => createAccountConsoleHandler(config),
    });
}
