...
export default defineComponent({
    ...
    setup() {
        const breadcrumbItems = useSectionBreadcrumb(LayoutSection.PATHS, { add: true });
        const sectionUrls = buildSectionURLs(LayoutSection.PATHS);

        const toast = useToast();
        const errorToast = useErrorToast();
        const handleFailed = (e: Error) => errorToast.show(e);

        const hasAddPermission = usePermissionCheck({ name: PermissionName.PATH_CREATE });
        ...
        const translationsApp = useTranslationsForNamespace(
            ...
            [
                { key: TranslatorTranslationAppKey.PATH_DESCRIPTION },
            ],
        );

        const translate = useTranslator();

        const handleDeleted = async (e: Path) => {
            if (toast) {
                toast.show({
                    variant: 'success',
    ...
    <div>
        ...
        <div class="mb-3 flex flex-wrap items-start justify-between gap-3">
            <div>
                ...
                <p class="sub-title">
                    {{ translationsApp.pathDescription }}
                </p>
            </div>
            <AContentAction
