...
export default defineComponent({
    ...
    async setup() {
        ...
        let resolved : Ref<{ session: Session, subjectName: string | null } | null> = ref(null);
        try {
            ...
            resolved = ref({ session: response.data, subjectName });
        } catch {
            await router.replace({ path: '/sessions' });
        }

        const entity = computed(() => resolved.value?.session ?? null);
        const subjectName = computed(() => resolved.value?.subjectName ?? null);
