    ...
    setup() {
        ...
        const revokeAll = async (group: ConsentGroup, reload: () => Promise<void>) => {
            ...
            try {
                ...
                // leave some scopes granted. The server caps the page size (the
                // consent schema's pagination.maxLimit), so loop fetch-and-delete
                // until no rows remain; each pass deletes everything it fetched,
                // so the loop converges. The iteration bound is a defensive
                // backstop only.
                for (let i = 0; i < 100; i++) {
                ...
                toast.show({
                    variant: 'success',
                    body: translations.consentRevokeAllSuccess,
                });
