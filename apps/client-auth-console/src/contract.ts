 ...
 * History:
 *
 * 3 - the `/authorize` payload carries `federatedLogin: { handle, providerId }`
 *     and the page has to redeem it at
 *     `POST /identity-providers/:id/login-complete`, presenting the challenge
 *     the login form kept in session storage (plan 094). The federated
 *     callback no longer mints the application's code, so a package that
 *     ignores the field strands every federated login on the login form. The
 *     interstitial route from version 2 stays exported but the host no longer
 *     renders it.
 *
