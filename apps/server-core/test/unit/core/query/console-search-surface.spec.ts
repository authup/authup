...
const CONSOLE_SEARCH_FIELDS : Record<string, string[]> = {
    ...
    // a folder's single-segment `name` is deliberately not filterable: the
    // full `path` covers it, so the console searches that and the label
    [EntityType.PATH]: ['path', 'displayName'],
