...
describe('OAuth2AuthorizationStateManager', () => {
    ...
    describe('verify', () => {
        it('should return state when IP matches', async () => {
            ...
            const id = await manager.save(state);
            const result = await manager.verify(id, { ip: '10.0.0.1', userAgent: 'TestAgent' });
        ...
        it('should throw when user agent does not match', async () => {
            const id = await manager.save({ ip: '10.0.0.1', userAgent: 'Chrome' });
            await expect(
                manager.verify(id, { ip: '10.0.0.1', userAgent: 'Firefox' }),
            ).rejects.toThrow(OAuth2Error);
        });
        ...
        it('should skip user agent check when payload has no userAgent', async () => {
            const id = await manager.save({ ip: '10.0.0.1' });
            const result = await manager.verify(id, { ip: '10.0.0.1', userAgent: 'anything' });
            expect(result.ip).toBe('10.0.0.1');
        });
