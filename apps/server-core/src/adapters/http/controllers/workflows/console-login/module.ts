 ...
 *
 * `access_denied` is the one the consoles render specially (an accessPolicyId
 * on the console client); the other two land on the same generic notice but
 * say something true in a log or a bug report.
 */
...
/**
 * A console's server-side login (plan 088): the kick and the code redemption.
 * Both exist so no OAuth2 token ever reaches the console's JavaScript. The
 * browser holds an opaque, `HttpOnly` credential naming its `auth_sessions`
 * row instead.
 *
...
export class ConsoleLogin {
    ...
    constructor(definition: ConsoleLoginDefinition, ctx: ConsoleLoginContext) {
        this.definition = definition;
        this.baseURL = ctx.options.baseURL;
     ...
     */
    async login(event: IAppEvent) : Promise<Response> {
        ...
        url.searchParams.set('client_id', this.definition.clientName);
        ...
        // The default the consoles' client-side kick sends, so a lingering
        // session still offers "continue as / use another account".
     ...
     */
    async callback(event: IAppEvent) : Promise<Response> {
        ...
        if (!isSameOriginRequest(event, this.baseURL, { logger: this.logger })) {
        ...
        // Read before consuming: one browser holds one login cookie per
        // console, so a second tab's kick replaces the first tab's id. A stale
        // callback that consumed first would burn the live login of the tab
        ...
        try {
            ...
            const grant = await client.token.createWithAuthorizationCode({
                ...
                client_id: this.definition.clientName,
        ...
        setSessionCookie(event, this.baseURL, secret, ttl);
    ...
    protected refuse(event: IAppEvent, error: `${OAuth2ErrorCode}`) : Response {
        ...
        const target = this.definition.refusalPath ?
            `${this.buildConsoleURL()}/${this.definition.refusalPath}` :
            this.buildConsoleURL();

        return sendRedirect(event, `${target}?${params.toString()}`);
    }
    ...
    protected basePath() : string {
        return getURLBasePath(this.baseURL);
    }
    ...
    protected buildCallbackURL() : string {
        return `${this.trimmedBaseURL()}/${this.definition.segment}/callback`;
    }
    ...
    protected isSecureBaseURL() : boolean {
        try {
            return new URL(this.baseURL).protocol === 'https:';
        } catch {
            return false;
        }
    }
     ...
     * request and two consoles keep separate logins in flight, and
     * `SameSite=Lax` because the return leg is a top-level navigation that
     * may come back through an external identity provider, which a `Strict`
     * cookie would not accompany. It holds nothing but the id of a single-use
     * cache entry.
     */
