...
import { renameForeignKey, renameIndex } from 'typeorm-extension';
import {
    FOREIGN_KEY_RENAMES,
    INDEX_RENAMES,
    invertRenames,
 ...
 *
 * Renames only: no table is rewritten, no row is read or written. The
 * helpers are no-ops when the target name is already in place, so the
 * migration passes over a schema that already matches and can be re-run
