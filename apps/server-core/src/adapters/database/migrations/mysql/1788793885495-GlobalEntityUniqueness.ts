...
export class GlobalEntityUniqueness1788793885495 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        `);
        await queryRunner.query(`
            CREATE UNIQUE INDEX \`IDX_auth_roles_global_name\` ON \`auth_roles\` (\`name\`, (COALESCE(\`client_id\`, '')), (COALESCE(\`realm_id\`, '')))
        `);
        await queryRunner.query(`
            CREATE UNIQUE INDEX \`IDX_auth_scopes_global_name\` ON \`auth_scopes\` (\`name\`, (COALESCE(\`realm_id\`, '')))
        `);
        await queryRunner.query(`
            CREATE UNIQUE INDEX \`IDX_auth_policies_global_name\` ON \`auth_policies\` (\`name\`, (COALESCE(\`realm_id\`, '')))
        `);
    }

    public async down(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`
            DROP INDEX \`IDX_auth_policies_global_name\` ON \`auth_policies\`
        `);
        await queryRunner.query(`
            DROP INDEX \`IDX_auth_scopes_global_name\` ON \`auth_scopes\`
        `);
        await queryRunner.query(`
            DROP INDEX \`IDX_auth_roles_global_name\` ON \`auth_roles\`
        `);
        await queryRunner.query(`
