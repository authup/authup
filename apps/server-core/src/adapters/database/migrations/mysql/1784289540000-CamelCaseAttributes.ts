...
async function renameKeys(
    ...
    for (const { value } of rows) {
        ...
        await queryRunner.query(
            `DELETE \`source\` FROM \`${table}\` AS \`source\` ` +
            `INNER JOIN \`${table}\` AS \`target\` ON \`source\`.\`${ownerColumn}\` = \`target\`.\`${ownerColumn}\` ` +
            `WHERE \`source\`.\`${valueColumn}\` = ? AND \`target\`.\`${valueColumn}\` = ?`,
            [value, next],
        );

        await queryRunner.query(
            `UPDATE \`${table}\` SET \`${valueColumn}\` = ? WHERE \`${valueColumn}\` = ?`,
            [next, value],
        );
    }
}
