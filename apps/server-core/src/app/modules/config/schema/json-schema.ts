...
/**
 * A registry as a JSON Schema (draft-07) document: one property per config
 * key in registry order, carrying the description, the static default and
 * the environment variable name (`x-authup-env`).
 *
...
export function buildSchemaJSONSchema<T>(
    ...
    for (const key of keys) {
        ...
        properties[key as string] = property;
    }
