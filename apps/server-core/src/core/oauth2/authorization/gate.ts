...
import { IdentityType, SessionAuthMethod } from '@authup/core-kit';
import {
...
import type {
    ...
    OAuth2AuthorizationGateContext,
    OAuth2AuthorizationGateInput,
    OAuth2AuthorizationGateResult,
    OAuth2AuthorizationOptions,
} from './types.ts';
import type { ISessionManager } from '../../authentication/index.ts';
import type { IUserAuthenticatorChallengeProvider } from '../../entities/index.ts';
...
export class OAuth2AuthorizationGate implements IOAuth2AuthorizationGate {
    ...
    constructor(ctx: OAuth2AuthorizationGateContext) {
    ...
    /**
     * Run the admission gates every authorization shares: identity present,
     * realm binding, the MFA backstop and step-up, prompt / max_age freshness
     * and the application access policy.
     *
     * @param data
     * @param identity
     * @param options
    ...
    async evaluate(
        ...
        // a clean login_required, never a TypeError.
        if (
            data.realm_id &&
        ...
        if (this.mfaChallengeProvider && identity.type === IdentityType.USER) {
            ...
            // urn:authup:mfa is a TRIGGER (Auth0/Keycloak stance), so the proof
        ...
        // issuance, so a denial is only revealed to a fully-authenticated
